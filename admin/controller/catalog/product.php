...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//Delete product from waiting table
			$this->load->model('extension/module/crawled_product');

			if (isset($_SESSION['crawled_product'])) {
				$this->model_extension_module_crawled_product->delete($_SESSION['crawled_product']['id']);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//Delete from waiting for update table
			if (isset($this->request->post['update_id'])) {
				$this->load->model('extension/module/crawled_product');

				$this->model_extension_module_crawled_product->deleteUpdate($this->request->post['update_id']);
			}
	...
	protected function getForm() {
		...
		// get crawled product info 
		$this->load->model('extension/module/crawled_product');
		$this->load->model('catalog/manufacturer');

		if (isset($_GET['crawled-id'])) {
			$id = intval($_GET['crawled-id']);

			$data['product'] = $this->model_extension_module_crawled_product->getProduct($id);
			$data['product_manufacturer'] = $this->model_catalog_manufacturer->getManufacturerByName($data['product']['product_manufacturer']);

			//Store it into session
			$_SESSION['crawled_product'] = $data['product'];
		}

		//Generate catalog number
		if (!isset($_GET['product_id'])) {
			$sql = 'SELECT model FROM ' . DB_PREFIX . 'product ORDER BY model DESC';
			$query = $db->prepare($sql);
			$query->execute([]);

			$productModel = $query->fetch(PDO::FETCH_ASSOC);

			if ($query->rowCount()) {
				$data['productModel'] = $productModel['model'] + 1;
			}
			else {
				$data['productModel'] = 10000;
			}
		}
