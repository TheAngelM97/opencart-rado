...
class ControllerExtensionShippingEcont extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/extension/shipping', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('extension/shipping/econt', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('extension/shipping/econt', 'token=' . $this->session->data['token'], 'SSL');
		...
		$office = $this->model_extension_shipping_econt->getOffice($data['econt_office_id']);
		...
		$data['offices'] = $this->model_extension_shipping_econt->getOfficesByCityId($data['econt_office_city_id'], $this->delivery_type);

		// APS
		$data['cities_aps'] = $this->model_extension_shipping_econt->getCitiesWithOffices($this->delivery_type, 1);

		$office_aps = $this->model_extension_shipping_econt->getOffice($data['econt_office_aps_id']);
		...
		$data['offices_aps'] = $this->model_extension_shipping_econt->getOfficesByCityId($data['econt_office_city_aps_id'], $this->delivery_type, 1);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('extension/shipping/econt.tpl', $data));
	}
	...
	private function validate() {
		...
		if (!$this->model_extension_shipping_econt->getCitiesWithOffices($this->delivery_type)) {
		...
		if (!isset($this->request->post['econt_addresses'])) {
			...
			foreach ($this->request->post['econt_addresses'] as $address) {
				if ($address['post_code'] && $address['city'] && ($address['quarter'] && $address['other'] || $address['street'] && $address['street_num'])) {
					if (!$this->model_extension_shipping_econt->validateAddress($address)) {
		...
		if (!isset($results_data['error']) && !$step) {
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->e)) {
						$this->model_extension_shipping_econt->deleteCountries();
						...
						foreach ($results->e as $country) {
							...
							$this->model_extension_shipping_econt->addCountry($country_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->zones)) {
						$this->model_extension_shipping_econt->deleteZones();
						...
						foreach ($results->zones->e as $zone) {
							...
							$this->model_extension_shipping_econt->addZone($zone_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->cities_regions)) {
						$this->model_extension_shipping_econt->deleteRegions();
						...
						foreach ($results->cities_regions->e as $region) {
							...
							$this->model_extension_shipping_econt->addRegion($region_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->cities_quarters)) {
						$this->model_extension_shipping_econt->deleteQuarters();
						...
						foreach ($results->cities_quarters->e as $quarter) {
							...
							$this->model_extension_shipping_econt->addQuarter($quarter_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->cities_street)) {
						$this->model_extension_shipping_econt->deleteStreets();
						...
						foreach ($results->cities_street->e as $street) {
							...
							$this->model_extension_shipping_econt->addStreet($street_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->offices)) {
						$this->model_extension_shipping_econt->deleteOffices();
						...
						foreach ($results->offices->e as $office) {
							...
							$this->model_extension_shipping_econt->addOffice($office_data);
						}
					}
			...
			if ($results) {
				if (isset($results->error)) {
					...
					if (isset($results->cities)) {
						$this->model_extension_shipping_econt->deleteCities();
						$this->model_extension_shipping_econt->deleteCitiesOffices();
						...
						foreach ($results->cities->e as $city) {
							...
							$this->model_extension_shipping_econt->addCity($city_data);
								...
								foreach ($city->attach_offices->children() as $shipment_type) {
									foreach ($shipment_type->children() as $delivery_type) {
										foreach ($delivery_type->office_code as $office_code) {
											...
											$this->model_extension_shipping_econt->addCityOffice($city_office_data);
										}
									}
								}
							}
						...
						$results_data['cities'] = $this->model_extension_shipping_econt->getCitiesWithOffices($this->delivery_type);
					}
				}
			} else {
				$results_data['error'] = $this->language->get('error_connect');
	...
	private function update_rs() { //for Return Solution
		...
		$this->model_extension_shipping_econt->updateTablesRS();
	...
	private function update_aps() { //for APS
		...
		$this->model_extension_shipping_econt->updateTablesAPS();
