				...
				foreach ($products as $product) {
					...
					for ($i = 0; $i < $product['quantity']; $i++) {
						$data['products'][] = array(
							...
							'price'      => round($this->currency->convert($this->currency->format($product['price'], $this->config->get('econt_currency'), $order_info['currency_id'], $order_info['currency_value'], FALSE), $order_info['currency_code'], $this->config->get('econt_currency')), 2)
						);
					}
				}
			}
	...
	private function generateLoading($data) {
		...
		if (isset($addresses[$address_id])) {
			...
			if ($this->config->get('econt_shipping_from') == 'OFFICE') {
				$sender_office = $this->model_extension_shipping_econt->getOffice($this->config->get('econt_office_id'));

				if ($sender_office) {
					$sender_office_code = $sender_office['office_code'];
				}
			} elseif ($this->config->get('econt_shipping_from') == 'APS') {
				$sender_office_aps = $this->model_extension_shipping_econt->getOffice($this->config->get('econt_office_aps_id'));
