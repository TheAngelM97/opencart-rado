...
class ModelExtensionModuleCrawledProduct extends Model
{
	...
	public function uploadInUpdates($product_id, $price, $quantity)
	{
		$sql = 'INSERT INTO crawled_updates (update_product_id, new_price, new_quantity) VALUES ('.$this->db->escape(trim($product_id)).', '.$this->db->escape(trim($price)).', "'.$this->db->escape(trim($quantity)).'")';
		if ($this->db->query($sql)) {
			return true;
		}
		return false;
