      ...
      <div class="panel-body">
        <form class="form-horizontal">
          <ul id="order" class="nav nav-tabs nav-justified">
            ...
            <li style="display: none;"><a href="#tab-payment" data-toggle="tab">3. <?php echo $tab_payment; ?></a></li>
            <li><a href="#tab-shipping" data-toggle="tab">4. <?php echo $tab_shipping; ?></a></li>
            <li id="button-total-refresh"><a href="#tab-total" data-toggle="tab">5. <?php echo $tab_total; ?></a></li>
          </ul>
          <div class="tab-content">
            ...
            <div class="tab-pane" id="tab-total">
              ...
              <fieldset>
                  ...
                  <div class="col-sm-10">
                    <div class="input-group">
                      <select name="shipping_method" id="input-shipping-method" class="form-control">
                        ...
                        <option value="<?php echo $shipping_code; ?>" selected="selected"><?php echo $order_shipping['title'] . ' - ' . $order_shipping['value'] . 'лв'; ?></option>
                        <?php } ?>
                      </select>
                      <span class="input-group-btn">
                ...
                <div class="form-group">
                	<label class="col-sm-2 control-label" for="input-shipping-price">Цена за доставка</label>
                	<div class="col-sm-10">
                		<div class="input-group">
                			<input type="number" name="shipping-price" id="input-shipping-price" value="<?= $order_shipping['value'] ?>" class="form-control">
                			<input type="hidden" name="shipping-price-id" id="order-total-id" value="<?= $order_shipping['order_total_id'] ?>">
                			<input type="hidden" name="order-id" value="<?= $order_shipping['order_id'] ?>" id="order-id">
                			<button type="button" id="shipping-price-button" class="btn btn-primary">Промени цената</button>
                		</div>
                	</div>
                </div>
                <div class="form-group required">
...
$('#button-total-refresh').on('click', function() {
    $('#button-customer').trigger('click', [ false ]);

});



$('#shipping-price-button').click(function(event) {
	let order_id = $('#order-id').val();
	console.log(order_id);
	let price = $('#input-shipping-price').val();
	let order_total_id = $('#order-total-id').val();

	$.ajax({
		url: 'index.php?route=sale/order/updateShippingPrice&token=<?= $token; ?>',
		type: 'POST',
		data: {order_id: order_id, order_total_id: order_total_id, price: price},
	})
	.done(function(json) {
		console.log("success");
		console.log(json);
...
$('#button-customer').on('click', function(event, tab = true) {
  $.ajax({
