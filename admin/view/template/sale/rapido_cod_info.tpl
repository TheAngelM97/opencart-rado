<?php echo $header; ?><?php echo $column_left; ?>
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right"><a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo $button_cancel; ?>" class="btn btn-default"><i class="fa fa-reply"></i></a></div>
      <h1><?php echo $heading_title; ?></h1>
      <ul class="breadcrumb">
        <?php foreach ($breadcrumbs as $breadcrumb) { ?>
        <li><a href="<?php echo $breadcrumb['href']; ?>"><?php echo $breadcrumb['text']; ?></a></li>
        <?php } ?>
      </ul>
    </div>
  </div>
  <div class="container-fluid">
    <?php if ($error_warning) { ?>
    <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> <?php echo $error_warning; ?>
      <button type="button" class="close" data-dismiss="alert">&times;</button>
    </div>
    <?php } ?>
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-list"></i> <?php echo $heading_title; ?></h3>
      </div>
      <div class="panel-body">
        <div class="tab-content">
            <table class="table table-bordered">
              <tr>
                <td><?php echo $text_tovaritelnica; ?></td>
                <td><?php echo $tovaritelnica; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_total_cod; ?></td>
                <td><?php echo $total_cod; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_receiver_city; ?></td>
                <td><?php echo $receiver_city; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_paid_date; ?></td>
                <td><?php echo $paid_date; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_company_id; ?></td>
                <td><?php echo $company_id; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_expense_order; ?></td>
                <td><?php echo $expense_order; ?></td>
              </tr>
              <tr>
                <td><?php echo $text_client_ref; ?></td>
                <td><?php echo $client_ref; ?></td>
              </tr>
            </table>
        </div>
      </div>
    </div>
  </div>
</div>
<?php echo $footer; ?> 